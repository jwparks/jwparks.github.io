...
현재에도 널리 사용되는 조명 모델인 Phong 반사 모델은 컴퓨터 과학자 Bui Tuong Phong이 1975년 출판된 본인의 박사학위 논문에서 처음 소개되었습니다.
...
보시는 것과 같이 물체의 최종 색(Light intensity)이 주변광(Ambient), 확산광(Diffuse), 그리고 반사광(Specular)의 합으로 결정되는 것을 볼 수 있습니다.
수학적으로 Phong 반사 모델은 다음과 같이 구현 할 수 있습니다.

$$ I_p = k_{a}I_{a} + \sum_{l \in \text{lights}} (k_{d}(\hat{L}_m \cdot \hat{N})I_{l,d} + k_{s}(\hat{R}_m \cdot \hat{V})^\alpha I_{l,s}) $$

이제 수식을 풀어서 설명해 보도록 하겠습니다. 
Phong 반사 모델을 이용해 계산 되는 물체의 최종적인 색 $I_p$은 주변광 $k_{a}I_{a}$, 
확산광 $k_{d}(\hat{L}_m \cdot \hat{N})I_{l,d}$, 
그리고 반사광 $k_{s}(\hat{R}_m \cdot \hat{V})^\alpha I_{l,s}$의 합으로 결정 됩니다.
