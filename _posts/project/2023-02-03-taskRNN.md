...
이에 실제 응용역학에서 가정해야 하는 모델 시스템은 더 낮은 추상화 수준을 가져야 합니다.
...
Dominant color에 따라 패치가 변하게 됩니다. 하지만 coherence 매우 level이 낮은 경우, 앞선 랜덤 패치와 구분이 되지 않을 수 있습니다.
...
위 코드는 인공신경망 학습을 위한 과제 데이터를 랜덤하게 생성시킵니다.
...
위 코드를 보시면 과제 시작 후 10-30 timestep 사이에서 타겟의 시각 단서가 주어지고(`target_onest_range`), 
랜덤한 패치의 색이 한쪽으로 결정되는 시점은 50-80 timestep 사이인 것을 확인 할 수 있습니다.
이 경우는 모델이 수행하는 과제이므로 이산화된 해당 timestep은 특정한 현실의 물리적인 시간에 해당하지는 않습니다.
따라서 여러분이은 이렇게 이산화된 timestep의  sampling rate를 조절하여 더 촘촘한 task를 구현 하실 수 있습니다.
높은 sampling rate 하에서 여러분은 모델로부터 반응 시간(reaction time)이나 기타 심리물리학적인 측정값을 더 정교하게 얻을 수 있지만,
timestep의 길이가 너무 길어질 경우 모델의 학습 효율이 매우 떨어진다는 점을 주의하시면 되겠습니다.

여기까지 여러분은 RNN 모델이 수행 할 수 있는 시각 판별 과제를 구현해 보았습니다. 이제 RNN 모델을 구현하고, 학습시켜 보도록 하겠습니다.
...
`h` 혹은 hidden state는 기본적인 RNN의 hidden state와 같이 이전 timestep에서 주어진 정보를 현재 혹은 미래로 전달하는데 사용됩니다.
이러한 hidden state는 매 timestep에서 recurrent하게 동작하여 주어진 입력 데이터 시퀀스의 맥락(context)을 추론하는데 사용 됩니다.
...
우리가 따로 두 state를 업데이트 해 줄 필요가 없다는 점을 알고 계시면 되겠습니다.
최종적으로 나온 output을 Fully connected layer에 통과시켜 Button response로 변환합니다. 
...
사실 모델의 Loss를 정의하고, 학습시키는 데에는 여러 방법이 있습니다. 
예전에는 지금 보시는 것 처럼 supervised learning을 통해 계산된 Loss를 바로 역전파하여 학습하는 방법이 널리 쓰였는데요.
제 개인적인 생각으로는 Deep reinforcement learning에서 사용하는 reward 기반의 학습 체계가 더 좋다고 생각 합니다.
...
이제 여러분은 학습된 모델을 새로운 테스트 데이터에 적용시켜 다양한 행동 데이터를 얻을 수 있습니다.
...
with torch.no_grad():
    ...
    targets = targets.to(device).cpu().numpy()
...
결과는 다음과 같습니다. 제시되는 체커보드 패치의 색에 따라 반응 시간(reaction time)을 측정하였습니다.
Chandrasekaran et al. (2017) 연구에서 제시된 짧은 꼬리 원숭이의 행동 데이터와 우리가 구현하고 학습시킨 데이터를 비교해 보면 유사한 결과를 확인 할 수 있습니다.
다시 말해 잘 훈련된 RNN 모델도 주어지는 시각 자극에 따라 응답의 속도가 바뀐다는 점을 시사합니다. 
여기선 응답 속도라는 말 보다는 응답을 하는데 필요한 time step의 수 라고 표현하는 것이 정확하겠습니다.

[![behavioral-results.png](https://i.postimg.cc/vZLj6VcT/behavioral-results.png)](https://postimg.cc/NKFb3L8v)
Figure 6. 짧은 꼬리 원숭이와 RNN 모델의 행동 데이터 비교
{: style="color:gray; font-size: 90%; text-align: center;"}
